...
var lottery = {
    ...
    _down: function() {
        var _this = this;
        if (_this.options.target === -1 || (_this.options.speed > _this.options.downMax && _this.options.target == _this._index())) {
            _this._stop();
        } else {
    ...
    _stop: function() {
        ...
        _this._enable();
        if (_this.options.target === -1) {
            _this.container.find('.lottery-unit.active').removeClass("active");
        } else if (_this.options.successCb) {
            _this.options.successCb.call(_this);
        }
    },
    ...
    beforeRoll: function() {
        ...
        if (_this.options.beforeRoll) {
            if (_this.options.beforeRoll.call(_this) === false) {
                _this._enable();
                return;
            }
        }
    ...
    // 转盘总格子数
    _count: function() {
        if (this.options.width < 3 || this.options.height < 3) {
