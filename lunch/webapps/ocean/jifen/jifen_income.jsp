...
$(document).ready(function(){
	...
	function ajaxRed(){
		...
		if (currentpage>maxpage){
		  ...
		  $.ajax({
		        type:'GET',
		        url:'${pageContext.request.contextPath}/jifen/jifenIncomeAjax',
		        data:{'pageNo':currentpage,'userId':${userId}},
		        dataType:'json',
		        timeout:10000,
		        success:function (jifenList) {
		        	for(var i = 0; i < jifenList.length; i++) {
		        		var jifen = jifenList[i];
						var html = '<div class="vip_money"><p class="hb_left"><img src="'+jifen.imageUrl+'" class="w100" />';
						if(jifen.cgfFlag){
							html +='<span class="vip_fc fontSize12">超高返</span>';
						}
						html +='</p>'+
						'<div class="hb_right"><p class="hb_right_p1 fontSize14">'+jifen.typeStr+'</p>';
						if(jifen.title){
							html +=	'<p class="hb_right_p2 fontSize14" >'+jifen.title+'</p>';
						}
						html +='<p class="hb_right_p3 fontSize14">'+jifen.srcTimeStr+'<span class="mx_right" ><span class="hong">'+jifen.jifen+'</span>积分</span></p>'+
						'</div></div>';
						$("#jifen_income").append(html);
		        	}
		        }
		  });
	}    
...
<body>
<div class="vip" id="jifen_income">
	<c:if test="${not empty jifenList}">
		<c:forEach items="${jifenList}" var="jifen" varStatus="num">
		<div class="vip_money">
	        <p class="hb_left"><img src="${jifen.imageUrl}"  class="w100" />
	        <c:if test="${jifen.cgfFlag}">
	        <span class="vip_fc fontSize12">超高返</span>	        
	        </c:if>
	        </p>
	        <div class="hb_right">
	            <p class="hb_right_p1 fontSize14">${jifen.typeStr}</p>
	            <p class="hb_right_p2 fontSize14" >${jifen.title}</p>
	...
	<c:if test="${empty jifenList }">
		...
		<br />您还没有积分支出，去兑换一个返利红包吧，<br/>红包可以在购物的时候抵现哦</div>		
	</c:if>
</div>
</body>
</html>
