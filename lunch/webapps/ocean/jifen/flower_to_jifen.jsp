...
<div class="vip">
	<div class="fyb_head">
        <p class="fyb_p1 fontSize14">可兑换鲜花</p>
        <p class="fyb_p2 fontSize14"><span class="fontSize28">${flower}</span>朵</p>
        <p class="fyb_p3 fontSize14">1朵鲜花=1个积分</p>
        <c:if test="${flower gt 0 }">
            <a href="javascript:void(0);" class="fyb_a fontSize14" onclick="duihuan()">全部兑换</a>
        </c:if>
        <c:if test="${flower le 0 }">
            <a href="javascript:void(0);" class="fyb_a fontSize14"  style="background:#999;">全部兑换</a>
        </c:if>
    </div>
    <h1 class="fyb_h1 fontSize16">积分兑换规则</h1>
    <p class="fyb_gz fontSize14">1.鲜花与积分的兑换比例为1:1<br />&nbsp;&nbsp;1朵鲜花可兑换为1个积分</p>
...
<!--弹出框 strat-->
    <div class="inte_tank" id="JS_tank">
    	<div id="log_1" style="display:none;">
        	<p class="jf_div_wz fontSize14">您确定要将您的鲜花<br />全部兑换为积分吗？</p>
        ...
        <div id="log_4"  style="display:none;">
        	<p class="jf_div_wz fontSize14"><img src="https://img.alicdn.com/imgextra/i1/2296013456/TB2RNgzeFXXXXbnXpXXXXXXXXXX_!!2296013456.png"  /><br />您的鲜花不够，不能兑换</p>
            <a href="#" class="jf_href" onclick="Close()">确定</a>
        </div>
    </div>
    ...
    function duihuansure(){  
    	     $("#log_1").hide();
		     $.post("${pageContext.request.contextPath}/jifen/flowerToJifenAjax",{"userId":${userId}},function(resp){
				 resp = $.parseJSON(resp);
				 if(resp.success){
					 $("#log_2").show();
				 }else{
					if(resp.status=='4'){//鲜花不够
						$("#log_4").show();
					}else{//兑换失败 
						$("#log_3").show();
					}
