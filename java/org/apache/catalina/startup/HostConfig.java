...
public class HostConfig
    ...
    public void lifecycleEvent(LifecycleEvent event) {
        ...
        try {
            ...
            if (host instanceof StandardHost) {
                setCopyXML(((StandardHost) host).isCopyXML());//default false
                setDeployXML(((StandardHost) host).isDeployXML());//default false
                setUnpackWARs(((StandardHost) host).isUnpackWARs());//default true
                setContextClass(((StandardHost) host).getContextClass());//default StandardContext
            }
    ...
    protected void deployDescriptor(ContextName cn, File contextXml) {
        ...
        Context context = null;//StandardContext
        ...
        try {
            ...
            synchronized (digesterLock) {
                try {
                    context = (Context) digester.parse(fis);//单节点(context)文件的解析
                } catch (Exception e) {
                    log.error(sm.getString(
	        ...
	        //ContextConfig PS:ContextConfig与HostConfig是两个比较重要的类 HostConfig涉及到web应用程序的加载 ContextConfig涉及到web.xml的加载
    ...
    protected void deployDirectory(ContextName cn, File dir) {
        ...
        try {
            ...
            //将会调用StandardContext对象的#start()方法
            host.addChild(context);
        } catch (Throwable t) {
            ExceptionUtils.handleThrowable(t);
    ...
    /**
     * 提及一下阅读源码的方法问题：读代码不要一行一行的读下去 涉及太多 有些代码就是要后读的 比如下面这个方法 刚阅读这个方法
     * 的时候 你只要阅读deployApps()这个方法就可以了 因为刚开始的时候 apps是空 host.undeployOldVersions也是false
     * 如果非得把checkResources checkUndeploy deployApps按顺序阅读的话 估计会比较耗时 而且会打消阅读源码的积极性
     * Check status of all webapps.
     */
    protected void check() {

        if (host.getAutoDeploy()) {//default true
            ...
            if (host.getUndeployOldVersions()) {// default false
            	//TODO
                checkUndeploy();
            }

            // Hotdeploy applications 热部署
            deployApps();
        }
    }
